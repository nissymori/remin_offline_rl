...
class ReBRACReMinEnsembleAgent(flax.struct.PyTreeNode):
    """ReBRAC with ensemble critics and ReMin actor optimization."""
    ...
    def actor_loss(self, batch, grad_params, rng):
        """Compute the ReBRAC actor loss with per-head ReMin over action retries."""
        batch_size, action_dim = batch['actions'].shape
        dist = self.network.select('actor')(batch['observations'], params=grad_params)
        actions = dist.mode()

        rng, remin_rng = jax.random.split(rng)
        action_samples = jnp.broadcast_to(
            actions,
            (self.config['num_remin_action_samples'], *actions.shape),
        )
        action_noise = jnp.clip(
            jax.random.normal(remin_rng, action_samples.shape) * self.config['actor_noise'],
            -self.config['actor_noise_clip'],
            self.config['actor_noise_clip'],
        )
        remin_actions = jnp.clip(action_samples + action_noise, -1, 1)
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='rebrac_remin_ensemble',
            lr=3e-4,
            batch_size=256,
